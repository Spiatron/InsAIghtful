...
export default function Chatbot() {
  ...
  const [chatHistory, setChatHistory] = useState([]); // history and chat variable
  const [userInput, setuserInput] = useState("   ");
  const [firstMessage, setFirstMessage] = useState(null);
  const [isChatbotOpen, setIsChatbotOpen] = useState(false); // Toggler variable

  // history and chat function
  if (firstMessage) {
    buttonHandler();
...
function adjustTextareaHeight(textarea, event) {
  if (event && event.keyCode === 8) { // Check if event exists and if the pressed key is the backspace key
  ...
  return (
    <div className={`show-chatbot ${isChatbotOpen ? 'open' : 'closed'}`}>
      <button className="chatbot-toggler" onClick={toggleChatbot}>
        <span>{isChatbotOpen ? <IoCloseOutline size={25} /> : <BsChatRightText size={25} />}</span>
      </button>
      {isChatbotOpen && (
        <div className="chatbot">
          ...
          <ul className="chatbox">
            {chatHistory.map((chat, index) => (
              <div key={index}>
                ...
                <li className="chat incoming"> <span><RiRobot2Line size={25} /></span>
                  <p> {chat.bot}</p>
                </li>
              </div>
            ))}
          ...
          <div className="chat-input show-">
            ...
            />
            <span id="send-btn" onClick={buttonHandler} className="material-symbols-outlined"><IoSend /></span>
          </div>
        </div>
      )}
    </div>
